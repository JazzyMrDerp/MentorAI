...
```
#/                  dashboard
#/math  #/ela       subject lesson list
#/lesson/:id        lesson + tutor chat
#/quiz/:id          quiz in progress
#/boss/:subject     boss battle
#/progress          progress
#/settings          settings
```

`parseHash` is total — every input resolves to something, and anything it cannot read falls
back to the dashboard and rewrites the address bar to match. A student's URL bar is
untrusted input like any other: ids are checked with `Number.isInteger(n) && n > 0` rather
than coerced, because `parseInt('7abc')` would have routed a typo to lesson 7.

`applyRoute` is the only thing that writes navigation state. Screens no longer set a page
and re-render; they set the hash, and the hash lands there — which is what keeps the
address bar and the screen from disagreeing on Back, Forward and reload, since all three
arrive by exactly the path a click does.

Two screens hold state the URL cannot carry, and they answer a cold link differently:

- **`#/quiz/:id` is resumable only while the attempt is in memory.** The answers so far,
  the hints left and the running score are module state in `quiz.ts`. Reloading or stepping
  Back into a quiz therefore lands on `#/lesson/:id` rather than silently restarting from
  question 1 with the score wiped.
- **`#/boss/:subject` always starts a fresh battle.** It belongs to no lesson and carries
  nothing to lose — it is a three-minute challenge over the whole subject, so the link is
  worth sharing.

Summary screens are deliberately transient. They have no URL of their own; the hash still
reads `#/quiz/7` underneath, so a reload resolves the route beneath rather than re-showing
a score for an attempt already written to IndexedDB.

Onboarding is unrouted. It exists precisely because there is no student yet, so there is
nothing to link to — every hash resolves to it until a profile exists.

Two things fell out of this. `render()` is now synchronous: every fetch a route needs is
awaited in `applyRoute` first, so the old hazard of awaiting between emptying `#app` and
appending to it — which let two renders each append a layout — is prevented by the
signature instead of by a comment asking for it. And a document-wide click delegator is
gone. It caught every `[data-page]`, `[data-action]` and `[data-route]` and fed the value
to the navigator as though it were a page name; most were not (`start-lesson-7`,
`continue-math`, `back`), so it matched nothing, changed nothing, and re-rendered anyway.
Every Start Lesson click rendered the app root twice. Three screens carried comments
explaining which attribute names they had to avoid to stay out of its way; those are gone
with it.
...
- **Test coverage stops short of the screens.** The sync engine, the local database, the
  schema migration, the proxy, the prompt builders, the output validators, the escaper, the
  router and the failure classifiers are covered — 241 cases. The screens themselves are
  not: the router's tests cover parsing and dispatch, not what gets painted afterwards.
